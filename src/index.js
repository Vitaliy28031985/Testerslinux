...
const createInput = () => {
   const inputListElements = testAry.map(
       ({ name, a, b, c, d, i, answerCorrect }) =>
         ...
         <input class="input" data-corrects="${answerCorrect}" name="${a.id}" value="${a.answer}" id="${a.id}" type="radio"><label class="label" for="${a.id}">${a.answer}</label>
      
         <input class="input" data-corrects="${answerCorrect}" name="${b.id}" value="${b.answer}" id="${b.id}" type="radio"><label class="label" for="${b.id}">${b.answer}</label>
      
         <input class="input" data-corrects="${answerCorrect}" name="${c.id}" value="${c.answer}" id="${c.id}" type="radio"><label class="label" for="${c.id}">${c.answer}</label>
